...
CURRENCY_SYMBOL_FALLBACKS = {
    "INR": "₹", "INDIAN RUPEE": "₹", "RUPEE": "₹", "RUPEES": "₹", "RS": "₹", "RS.": "₹",
    "USD": "$", "US DOLLAR": "$", "DOLLAR": "$", "EUR": "€", "EURO": "€",
    "GBP": "£", "POUND": "£", "POUND STERLING": "£", "AED": "د.இ", "DIRHAM": "د.இ", "": "",
}
...
def parse_ledgers(root, group_map):
    ...
    for elem in root.iter():
        ...
        row = {"MasterID": clean_text(elem.get("MASTERID")) or direct_child_text(elem, "MASTERID"), "Name": name, "PrimaryGroup": gi.get("PrimaryGroup") or first_non_empty_text(elem, ["PRIMARYGROUP"]) or first_descendant_text(elem, "PRIMARYGROUP"), "Nature": "", "NatureOfGroup": gi.get("Nature", ""), "PAN": first_non_empty_text(elem, ["INCOMETAXNUMBER", "PAN"]) or first_descendant_text(elem, "INCOMETAXNUMBER"), "StartingFrom": direct_child_text(elem, "STARTINGFROM") or first_descendant_text(elem, "STARTINGFROM"), "CurrencyNameRaw": direct_child_text(elem, "CURRENCYNAME") or first_descendant_text(elem, "CURRENCYNAME"), "CurrencyFormalNameRaw": direct_child_text(elem, "CURRENCYFORMALNAME") or first_descendant_text(elem, "CURRENCYFORMALNAME"), "CurrencySymbolRaw": direct_child_text(elem, "CURRENCYSYMBOL") or first_descendant_text(elem, "CURRENCYSYMBOL"), "CurrencyOriginalSymbolRaw": direct_child_text(elem, "CURRENCYORIGINALSYMBOL") or first_descendant_text(elem, "CURRENCYORIGINALSYMBOL"), "StateName": direct_child_text(elem, "STATENAME") or first_descendant_text(elem, "STATENAME"), "Parent": parent, "PartyGSTIN": first_non_empty_text(elem, ["PARTYGSTIN", "GSTIN"]) or first_descendant_text(elem, "PARTYGSTIN"), "OpeningBalance": to_float(first_non_empty_text(elem, ["OPENINGBALANCE"]) or first_descendant_text(elem, "OPENINGBALANCE")), "ClosingBalance": to_float(first_non_empty_text(elem, ["CLOSINGBALANCE"]) or first_descendant_text(elem, "CLOSINGBALANCE"))}
...
root = parse_xml_root(post_to_tally(url, build_ledger_request_xml(sel_comp)))
