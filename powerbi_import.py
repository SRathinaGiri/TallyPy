...
# 4. STOCK VOUCHERS (Inventory Entries)
sv_xml = f"<ENVELOPE><HEADER><VERSION>1</VERSION><TALLYREQUEST>EXPORT</TALLYREQUEST><TYPE>COLLECTION</TYPE><ID>SV</ID></HEADER><BODY><DESC><STATICVARIABLES><SVEXPORTFORMAT>$$SysName:XML</SVEXPORTFORMAT><SVCURRENTCOMPANY>{escape(COMPANY)}</SVCURRENTCOMPANY><SVFROMDATE TYPE='Date'>{escape(FROM_DATE)}</SVFROMDATE><SVTODATE TYPE='Date'>{escape(TO_DATE)}</SVTODATE></STATICVARIABLES><TDL><TDLMESSAGE><COLLECTION NAME=\"SV\"><TYPE>Voucher</TYPE><FETCH>Date, VoucherTypeName, VoucherNumber, Narration, InventoryEntries.*, AllInventoryEntries.*, InventoryEntriesIn.*, InventoryEntriesOut.*</FETCH></COLLECTION></TDLMESSAGE></TDL></DESC></BODY></ENVELOPE>"
sv_rows = []
try:
    ...
    for v in ET.fromstring(xml_cleanup(rsv.text)).iter():
        ...
        for ent in inv_nodes:
            ...
            qty = abs(float(to_decimal(direct_child_text(ent, "BILLEDQTY"))))
            amt = abs(float(to_decimal(direct_child_text(ent, "AMOUNT"))))
            sv_rows.append({
