...
"%POWERSHELL_EXE%" -NoProfile -ExecutionPolicy Bypass -Command "$key = 'HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\TallyQtExporter'; New-Item -Path $key -Force | Out-Null; Set-ItemProperty -Path $key -Name DisplayName -Value $env:APP_NAME; Set-ItemProperty -Path $key -Name DisplayVersion -Value '0.1.9'; Set-ItemProperty -Path $key -Name Publisher -Value 'TallyXML'; Set-ItemProperty -Path $key -Name InstallLocation -Value $env:INSTALL_DIR; Set-ItemProperty -Path $key -Name DisplayIcon -Value $env:TARGET_EXE; Set-ItemProperty -Path $key -Name UninstallString -Value ([char]34 + $env:UNINSTALL_CMD + [char]34); New-ItemProperty -Path $key -Name NoModify -Value 1 -PropertyType DWord -Force | Out-Null; New-ItemProperty -Path $key -Name NoRepair -Value 1 -PropertyType DWord -Force | Out-Null"
if errorlevel 1 exit /b 1
