        ...
        try:
            ...
            for voucher in root.iter():
                ...
                for inv in inv_nodes:
                    ...
                    is_in = direct_child_text(inv, "ISDEEMEDPOSITIVE").upper() == "YES"
                    ...
                    sv_rows.append({"Date": v_date, "VoucherTypeName": v_type, "VoucherNumber": v_number, "StockItemName": item_name.strip(), "BilledQty": q_val if is_in else -q_val, "Rate": to_float(direct_child_text(inv, "RATE")), "Amount": float(a_val if is_in else -a_val), "GodownName": gn, "BatchName": bn, "VoucherNarration": v_narration, "CompanyName": v_company, "FromDate": format_tally_date(f_dt), "ToDate": format_tally_date(t_dt)})
